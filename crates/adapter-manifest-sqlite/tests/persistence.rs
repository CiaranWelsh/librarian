...
fn rows_survive_reopen() {
...

    {
        ...
        m.record(&SourceId("alpha".into()), "extract", ManifestStatus::Success, 1, None, None).unwrap();
        m.record(&SourceId("beta".into()), "embed", ManifestStatus::Failed, 3, Some("net"), None).unwrap();
    }
    ...
    let beta = get_row(&m2, &SourceId("beta".into()), "embed").unwrap().unwrap();
...
fn idempotent_reopen_does_not_destroy_rows() {
    ...
    m1.record(&SourceId("alpha".into()), "extract", ManifestStatus::Success, 1, None, None).unwrap();
    drop(m1);
