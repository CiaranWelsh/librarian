...
use librarian_domain::{
    Chunker, ContentType, Document, Extractor, Indexer, SourceHash, SourceId,
};
...
fn code_chunks_carry_both_text_and_code_named_vectors() {
    ...
    let Ok(ix) = QdrantIndexer::open_with_extra_slot(
        &url(), &collection, dim_text, Some(("code".to_string(), dim_code)),
    ) else { eprintln!("skip: no Qdrant"); return; };

    let text_vecs: Vec<Vec<f32>> = chunks.iter().map(|_| vec![0.1; dim_text as usize]).collect();
    let code_vecs: Vec<Vec<f32>> = chunks.iter().map(|_| vec![0.2; dim_code as usize]).collect();
    ...
    let hits_text = ix.search(&vec![0.1; dim_text as usize], 1, None).expect("text search");
...
fn cross_content_type_query_filters_code_or_book() {
    ...
    let book_chunk = Chunk {
        chunk_id: ChunkId("b#0".into()), source_id: SourceId("book-a".into()),
        chunk_index: 0, text: "hexagonal architecture".into(),
    ...
    let code_chunk = {
        ...
        CodeChunker::new().chunk(&doc, ext).unwrap().into_iter().next().unwrap()
    };
