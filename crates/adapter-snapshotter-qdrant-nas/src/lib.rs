//! Snapshot adapter: Qdrant native snapshot API → push to NAS path → optional
//! retention. v1 uses a local directory as the "NAS" target; production
//! deployment mounts the NAS or copies via HTTPS/SCP — same shape, different
//! transport.
...
mod tests {
    ...
    fn prune_keeps_newest_n_files() {
        ...
        for i in 0..5 {
            ...
            // Stagger mtime by touching each file with progressively newer time.
            // Sleep to ensure distinct mtimes on coarse-resolution filesystems.
