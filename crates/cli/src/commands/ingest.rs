...
pub fn cmd_ingest(config_path: &Path, input: &Path) -> Result<(), String> {
    ...
    let outcomes = match (cfg.ingest.extractor.as_str(), &cfg.embedder) {
        ...
        ("text", EmbedderConfig::Openai { model, dimensions, batch_size }) => {
            ...
            run_ingest(&cfg, TextExtractor::new(), BlankLineChunker::new(), emb, &docs)?
        }
        ("pdf", EmbedderConfig::Openai { model, dimensions, batch_size }) => {
            let emb = openai(model, *dimensions, *batch_size)?;
            run_ingest(&cfg, PdfExtractor::new(), BlankLineChunker::new(), emb, &docs)?
        }
        ("code", EmbedderConfig::Openai { model, dimensions, batch_size }) => {
            let emb = openai(model, *dimensions, *batch_size)?;
        ...
        ("pdf", EmbedderConfig::Voyage { model, dimensions, batch_size }) => {
            let emb = voyage(model, *dimensions, *batch_size)?;
            run_ingest(&cfg, PdfExtractor::new(), BlankLineChunker::new(), emb, &docs)?
        }
...
fn openai(model: &str, dimensions: usize, batch_size: Option<usize>) -> Result<OpenAiEmbedder, String> {
    OpenAiEmbedder::from_env(OpenAiConfig {
        model: model.into(), dimensions,
        endpoint: None, batch_size, timeout: None,
    }).map_err(|e| e.to_string())
}

fn voyage(model: &str, dimensions: usize, batch_size: Option<usize>) -> Result<VoyageEmbedder, String> {
...
fn run_ingest<E, Ch, Em>(
    cfg: &Config, extractor: E, chunker: Ch, embedder: Em, docs: &[Document],
    ...
    Em: Embedder,
{
    ...
    let runner = BatchRunner {
        pipeline: Pipeline { extractor, chunker, embedder, indexer },
        manifest, cache,
    };
