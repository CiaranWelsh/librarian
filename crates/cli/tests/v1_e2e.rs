...
"#,
        ...
        pub fn ingest(&self, input: &Path) -> Result<IngestSummary, String> {
            let out = self.cli().args(["ingest", "--config"]).arg(&self.config).arg(input)
                .output().map_err(|e| e.to_string())?;
            if !out.status.success() {
                return Err(format!("ingest failed: {}",
            ...
            Ok(IngestSummary { success_count, fail_count, raw_stdout: stdout })
        }

        pub fn status(&self) -> Result<CollectionStatus, String> {
            let out = self.cli().args(["status", "--config"]).arg(&self.config)
                .output().map_err(|e| e.to_string())?;
            if !out.status.success() {
                return Err(format!("status failed: {}", String::from_utf8_lossy(&out.stderr)));
            }
            let stdout = String::from_utf8_lossy(&out.stdout).to_string();
            ...
            Ok(CollectionStatus { points, success_rows: s, cached_rows: c, failed_rows: f })
        }

        pub fn remove(&self, source_id: &str) -> Result<(), String> {
            let out = self.cli().args(["remove", "--config"]).arg(&self.config)
        ...
        pub fn snapshot(&self) -> Result<SnapshotId, String> {
            let out = self.cli().args(["snapshot", "--config"]).arg(&self.config)
                .output().map_err(|e| e.to_string())?;
            if !out.status.success() {
                return Err(String::from_utf8_lossy(&out.stderr).to_string());
            }
            ...
            let id = stdout
                ...
                .next().ok_or("snapshot id not in stdout")?
                .to_string();
            Ok(SnapshotId(id))
        }

        pub fn restore(&self, id: &SnapshotId) -> Result<(), String> {
            let out = self.cli().args(["restore", "--config"]).arg(&self.config).arg(&id.0)
                .output().map_err(|e| e.to_string())?;
            if !out.status.success() {
                return Err(String::from_utf8_lossy(&out.stderr).to_string());
            }
        ...
        pub fn fleet_start(&self, name: &str) -> Result<(), String> {
            let out = self.cli().args(["start", name, "--config"]).arg(&self.config)
                .output().map_err(|e| e.to_string())?;
            if !out.status.success() {
                return Err(String::from_utf8_lossy(&out.stderr).to_string());
            }
        ...
        pub fn fleet_stop(&self, name: &str) -> Result<(), String> {
            let out = self.cli().args(["stop", name])
                .output().map_err(|e| e.to_string())?;
            if !out.status.success() {
                return Err(String::from_utf8_lossy(&out.stderr).to_string());
            }
        ...
        pub fn fleet_list(&self) -> Result<Vec<FleetEntry>, String> {
            ...
            for line in stdout.lines() {
                ...
                if let Some(name) = line.split('\t').next() {
                    ...
                    let status = line.split('\t').find_map(|p| p.strip_prefix("status="))
                        .unwrap_or("").to_string();
                    let pid = line.split('\t').find_map(|p| p.strip_prefix("pid="))
                        .and_then(|s| s.parse().ok());
                    entries.push(FleetEntry { name: name.to_string(), status, port, pid });
                }
            }
            Ok(entries)
        }
    ...
    /// through the CLI.
    pub struct QdrantProbe { pub indexer: QdrantIndexer }

    impl QdrantProbe {
        pub fn open(collection: &str, dim: u64) -> Self {
            Self { indexer: QdrantIndexer::open(&qdrant_url(), collection, dim).expect("qdrant") }
        }

        pub fn point_count(&self) -> u64 { self.indexer.count().unwrap_or(0) }

        pub fn wipe_source(&self, source_id: &str) {
            let _ = self.indexer.delete_by_source_id(&SourceId(source_id.into()));
        }
    }

    // ─── McpHarness ──────────────────────────────────────────────────────

    pub struct McpHarness { child: Child, stdin: ChildStdin, stdout: BufReader<ChildStdout> }
    ...
    impl McpHarness {
        pub fn spawn(config: &Path) -> Self {
            let mut child = std::process::Command::new(collection_bin())
                ...
                .spawn().expect("spawn librarian-collection");
        ...
        pub fn search(&mut self, query: &str, k: u64) -> Vec<McpHit> {
            let resp = self.request(1, "tools/call", json!({
                "name":"search","arguments":{"query":query,"k":k}
            }));
            let body = resp["result"]["content"][0]["text"].as_str().unwrap_or("{}");
            let parsed: Value = serde_json::from_str(body).unwrap_or(Value::Null);
            parsed["hits"].as_array().cloned().unwrap_or_default().into_iter().map(|h| McpHit {
        ...
        pub fn list_documents(&mut self) -> Vec<String> {
            let resp = self.request(2, "tools/call", json!({
                "name":"list_documents","arguments":{}
            }));
            let body = resp["result"]["content"][0]["text"].as_str().unwrap_or("{}");
            let parsed: Value = serde_json::from_str(body).unwrap_or(Value::Null);
            parsed["documents"].as_array().cloned().unwrap_or_default().into_iter()
    ...
    impl FixtureBuilder {
        ...
        pub fn with_paragraphs(mut self, name: &str, n: usize) -> Self {
            let body = (0..n).map(|i| format!("paragraph {i}")).collect::<Vec<_>>().join("\n\n");
    ...
    fn parse_kv(s: &str, key: &str) -> u64 { parse_after(s, key).parse().unwrap_or(0) }
}
...
fn e2e_first_time_ingestion_lands_chunks_in_qdrant() {
    let Some(h) = LibrarianHarness::fresh("ingest") else { eprintln!("skip: no Qdrant"); return; };
...
fn e2e_second_ingest_hits_cache_and_yields_no_new_points() {
    let Some(h) = LibrarianHarness::fresh("idem") else { eprintln!("skip: no Qdrant"); return; };
    let fixtures = FixtureBuilder::new().with_paragraphs("a.txt", 3).write_to(h.workdir.path());
    ...
    assert_eq!(before, after, "deterministic IDs + cache hits = no new points");
    let s = h.status().expect("status");
...
fn e2e_modifying_a_file_drops_orphaned_chunks_after_re_ingest() {
    let Some(h) = LibrarianHarness::fresh("update") else { eprintln!("skip: no Qdrant"); return; };
    let fixtures = FixtureBuilder::new().with_paragraphs("a.txt", 5).write_to(h.workdir.path());
    ...
    assert_eq!(h.probe().point_count(), 2, "no orphaned points from prior 5-chunk version");
}
...
fn e2e_remove_drops_chunks_for_one_source_only() {
    let Some(h) = LibrarianHarness::fresh("remove") else { eprintln!("skip: no Qdrant"); return; };
    let fixtures = FixtureBuilder::new()
        .with_paragraphs("a.txt", 3)
        .with_paragraphs("b.txt", 2)
        .write_to(h.workdir.path());
...
fn e2e_snapshot_then_restore_recovers_collection_state() {
    let Some(h) = LibrarianHarness::fresh("snap") else { eprintln!("skip: no Qdrant"); return; };
    let fixtures = FixtureBuilder::new().with_paragraphs("a.txt", 4).write_to(h.workdir.path());
    h.ingest(&fixtures).expect("ingest");
    assert_eq!(h.probe().point_count(), 4);

    let snap_id = h.snapshot().expect("snapshot");
    assert!(h.nas_dir().join(&snap_id.0).exists(), "snapshot file landed on NAS");

    // Wipe: arrange teardown via direct API, not by removing through CLI
    // (which would invoke the runner pipeline).
    h.probe().wipe_source(&fixtures.join("a.txt").display().to_string());
    assert_eq!(h.probe().point_count(), 0, "collection wiped");

    h.restore(&snap_id).expect("restore");
    assert_eq!(h.probe().point_count(), 4, "snapshot restored prior 4 chunks");
}
...
fn e2e_snapshot_retention_prunes_old_files_on_nas() {
    let Some(h) = LibrarianHarness::fresh("retention") else { eprintln!("skip: no Qdrant"); return; };
    ...
    let fixtures = FixtureBuilder::new().with_file("a.txt", "x").write_to(h.workdir.path());
...
fn e2e_fleet_start_status_stop_walks_lifecycle() {
    let Some(h) = LibrarianHarness::fresh("fleet") else { eprintln!("skip: no Qdrant"); return; };

    h.fleet_start("primary").expect("start");
    let entries = h.fleet_list().expect("list");
    let primary = entries.iter().find(|e| e.name == "primary").expect("primary present");
    ...
    let primary_after = entries_after.iter().find(|e| e.name == "primary").expect("still listed");
...
fn e2e_mcp_search_returns_relevant_hits_for_ingested_text() {
    let Some(h) = LibrarianHarness::fresh("mcp") else { eprintln!("skip: no Qdrant"); return; };
    let fixtures = FixtureBuilder::new()
        .with_file("dragon.txt", "the dragon was huge\n\nthe knight was brave\n\nthe king was nervous")
        .write_to(h.workdir.path());
    h.ingest(&fixtures).expect("ingest");
    ...
    assert!(docs.iter().any(|s| s.contains("dragon.txt")), "list_documents reflects ingest");
}
