...
use librarian_domain::{
    AdapterIdentity, ConfigHash, ContentType, Document, Embedder, EmbedderError, Extractor,
...
struct FailingEmbedder { marker: &'static str, inner: StubEmbedder }
impl AdapterIdentity for FailingEmbedder {
    fn name(&self) -> &str { "fail-emb" }
    fn version(&self) -> StageVersion { StageVersion("v".into()) }
    fn config_hash(&self) -> ConfigHash { ConfigHash("c".into()) }
}
...
fn embedder_failure_writes_no_chunks_for_failed_doc_in_qdrant() {
    ...
    let runner = BatchRunner {
        pipeline: Pipeline {
            ...
            embedder: FailingEmbedder { marker: "d2", inner: stub },
            indexer: ix,
        },
    ...
    assert_eq!(runner.pipeline.indexer.count_by_source(&SourceId("d2".into())).unwrap(), 0);
