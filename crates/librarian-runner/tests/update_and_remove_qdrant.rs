...
struct ScriptedExtractor { next: RefCell<Vec<&'static str>> }
...
impl Extractor for ScriptedExtractor {
    ...
    fn extract(&self, _: &Document) -> Result<ExtractedText, Self::Error> {
        ...
        Ok(ExtractedText { spans: vec![TextSpan {
            kind: SpanKind::Paragraph, text: body.into(), page: None, byte_range: 0..body.len(),
        }]})
    }
}
...
fn update_drops_orphan_chunks_in_qdrant() {
    ...
    let runner = BatchRunner {
        pipeline: Pipeline {
            extractor: ScriptedExtractor { next: RefCell::new(vec![
                "p0\n\np1\n\np2\n\np3\n\np4",
                "p0\n\np1\n\np2",
            ])},
    ...
    assert_eq!(runner.pipeline.indexer.count_by_source(&SourceId("d0".into())).unwrap(), 5);

    runner.ingest_batch(&[doc("d0", "h-edited")]);
    assert_eq!(runner.pipeline.indexer.count_by_source(&SourceId("d0".into())).unwrap(), 3);
...
fn explicit_remove_drops_all_chunks_in_qdrant() {
    ...
    let runner = BatchRunner {
        pipeline: Pipeline {
            extractor: ScriptedExtractor { next: RefCell::new(vec!["p0\n\np1\n\np2"]) },
            chunker: BlankLineChunker::new(),
            embedder: stub,
            indexer: ix,
        },
    ...
    assert_eq!(runner.pipeline.indexer.count_by_source(&SourceId("d0".into())).unwrap(), 0);
